...
import { GetWorkflowExecutionWithPhases } from '@/actions/workflows/GetWorkExecutionWithPhases';
...
export default function ExecutionViewer({initialData,}:{initialData:ExecutionData}) {
  ...
  return <div className='flex w-full h-full'><aside className='w-[440px] min-w-[440px] max-w-[440px] border-r-2 border-separate flex flex-grow flex-col overflow-hidden'>
    ...
    <Separator/><div className='overflow-auto h-full px-2 py-4'>{query.data?.phases.map((phase,index)=>(
        ...
        <PhaseStatusBadge status={phase.status as ExecutionPhaseStatus}/></Button>
    ))}</div></aside><div className='flex w-full h-full'>{isRunning && ( 
      <div className='flex items-center flex-col gap-2 justify-center h-full w-full'><p className='font-bold'>Run is in the progress,please wait</p></div>
    )} {!isRunning && !selectedPhase && ( 
      <div className='flex items-center flex-col gap-2 justify-center h-full w-full'><div className='flex flex-col gap-1 text-center'><p className='font bold'>No phase selected</p><p className='text-sm text-muted-foreground'>Select a phase to view details</p></div></div>
    )}
...
function ExecutionLabel({icon,label,value}:{icon:LucideIcon,label:ReactNode,value:ReactNode}){
    const Icon=icon
    return (
        <div className='flex justify-between items-center py-2 px-4 text-sm'><div className="text-muted-foreground flex items-center gap-2"><Icon size={20} className='stroke-muted-foreground/80'/>
    <span>{label}</span></div><div className="font-semibold capitalize flex gap-2 items-center">{value}</div></div>
    )
}
...
function LogViewer({logs}:{logs:ExecutionLog[]|undefined}){
          ...
          <TableBody>
            {logs.map((log)=>(
                <TableRow key={log.id} className='text-muted-foreground'>
                  ...
                  <TableCell width={80} className={cn("uppercase text-xs font-bold p-[3px] pl-4",log.logLevel as LogLevel ==="error" && "text-destructive",log.logLevel as LogLevel==="info" && "text-primary")}>{log.logLevel}</TableCell>
  ...
  //we can't show output of launch browser even though it generates output as it is the browser but we can't save that in JSON object that's why instead of saving it in the inputs and outputs we are saving it in execution environment within browser variable that has a browser typt that we are importing from puppetter